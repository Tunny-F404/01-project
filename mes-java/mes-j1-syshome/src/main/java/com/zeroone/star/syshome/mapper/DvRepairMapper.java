...
public interface DvRepairMapper extends BaseMapper<DvRepair> {
    ...
    @Select("SELECT 'pro_task' AS eventType, '生产任务' AS eventTypeName, t.task_id AS id " +
            ...
            "WHERE uw.user_name = t.end_time = NULL AND uw.user_name = #{userName} " +
            "UNION " +
            "SELECT 'pro_feedback' AS eventType, '生产报工表审批' AS eventTypeName,f.record_id AS id " +
            "FROM pro_feedback f " +
            "WHERE f.user_name = #{userName} AND (f.status = '审批中' OR f.status = '已确认') " +
            "UNION " +
            "SELECT 'dv_repair' AS eventType, '设备维修' AS eventTypeName, f.repair_id AS id " +
            "FROM dv_repair f " +
            "JOIN dv_repair_line rl ON r.repair_id = rl.repair_id " +
            "WHERE r.accepted_by = #{userName} AND r.finish_date = NULL " +
            "UNION " +
            "SELECT 'dv_repair' AS eventType, '设备维修检验' AS eventTypeName, f.repair_id AS id " +
            "FROM dv_repair f JOIN dv_repair_line rl ON r.repair_id = rl.repair_id " +
            "WHERE r.confirm_by = #{userName} AND r.finish_date = NULL " +
            "UNION " +
            "SELECT 'qc_ipqc' AS eventType, '过程检验' AS eventTypeName, i.ipqc_id AS id " +
            "FROM qc_ipqc i " +
            "WHERE i.inspector = #{userName} AND (i.status = '审批中' OR i.status = '已确认') " +
            "UNION " +
            ...
            "FROM qc_oqc o " +
            "WHERE o.inspector = #{userName} AND (o.status = '审批中' OR o.status = '已确认') ")
