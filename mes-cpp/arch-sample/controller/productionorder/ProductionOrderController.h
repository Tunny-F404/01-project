...
public:
	...
	// 3.2 定义查询接口处理
	ENDPOINT(API_M_GET, "/productionorder/query-production-bom-order", queryBomOrder, QUERIES(QueryParams, queryParams), API_HANDLER_AUTH_PARAME) {
		// 解析查询参数为Query领域模型
		API_HANDLER_QUERY_PARAM(query, ExportBomOrderQuery, queryParams);
		// 呼叫执行函数响应结果
		API_HANDLER_RESP_VO(execQueryBomOrder(query));
	}
private:
	// 3.3 演示分页查询数据
	ExportBomOrderPageJsonVO::Wrapper execQueryBomOrder(const ExportBomOrderQuery::Wrapper& query);
