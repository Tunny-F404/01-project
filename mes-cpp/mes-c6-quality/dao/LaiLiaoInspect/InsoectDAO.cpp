...
//查询详情
Inspect_detailDO InspectDAO::selectWithID(const uint64_t iqc_id)
{
	stringstream sql;
	...
	string sqlStr = sql.str();
	std::list<Inspect_detailDO> A = sqlSession->executeQuery<Inspect_detailDO, Inspect_detailMapper>(sqlStr, mapper, "%ull", iqc_id);
	if (A.size() == 0) return {};
	return A.front();
}
...
{                                  //`iqc_id`,
	string sql = "INSERT INTO `qc_iqc` ( `iqc_code`,`iqc_name`,`template_id`, `vendor_id`, `vendor_code`,`vendor_name`,`vendor_nick`,`vendor_batch`,`item_id`,`item_code`,`item_name` ,`quantity_recived`,`quantity_check`,`quantity_unqualified`,`check_result`,`recive_date`,`inspect_date`,`inspector`,`status`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?，?)";
	return sqlSession->executeInsert(sql, "%s%s%ull%ull%s%s%s%s%ull%s%s%d%i%i%s%s%s%s%s",
		//iObj.getLine_id(),
		iObj.getIqc_code(),
		iObj.getIqc_name(),
		iObj.getTemplate_id(),
