...
public: // 定义接口
	...
	API_HANDLER_ENDPOINT(API_M_DEL, "/equipment-ledger/remove-device", removeDevice, BODY_DTO(RemoveDeviceDTO::Wrapper, dto), execRemoveDevice(dto))
private: // 定义接口执行函数

	// 前端发过来的dto由这个函数执行
	BooleanJsonVO::Wrapper execRemoveDevice(const RemoveDeviceDTO::Wrapper& dto);
};
