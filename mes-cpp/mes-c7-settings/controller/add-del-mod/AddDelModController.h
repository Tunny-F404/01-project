...
public:
	...
	// 3.2 定义新增接口处理
	ENDPOINT(API_M_POST, "/add-del-mod", addWarehouse, BODY_DTO(AddDelModDTO::Wrapper, dto), API_HANDLER_AUTH_PARAME) {
		// 呼叫执行函数响应结果
	...
	// 3.2 定义修改接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_PUT, "/add-del-mod", modifyWarehouse, BODY_DTO(AddDelModDTO::Wrapper, dto), execModifyWarehouse(dto));
	...
	// 3.2 定义删除接口处理
	API_HANDLER_ENDPOINT_AUTH(API_M_DEL, "/add-del-mod/{warehouse_id}", removeWarehouse, PATH(UInt64, warehouse_id), execRemoveWarehouse(warehouse_id));
